...
if __name__ == "__main__":
    ...
    p.add('--batch_size', type=int, default=100, help='# of tables in a batch')
    ...
    p.add('--cross_validation', type=int_or_none, default=None, help='number of cross validation')

    p.add('--model_name', type=str, default='')
    ...
    logging_name = '{}'.format(args.model_name)
    ...
    if cv is None:
        ...
        random_int_list = random.sample(range(1, cv + 1), round(cv * 0.2))
    ...
    result_df.to_csv(join(logging_path, "prediction.csv"))
    print("testset predictioin saved")
    ...
    pretrained_name = '{}.pt'.format(logging_name)
